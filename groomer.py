...
# 2024: Modified for GhostRiderCoin by BartMan__X
import sys
import operator
...
parser.add_argument('-o', '--max_amt_per_output', type=Decimal, default=Decimal('10000'),
  help='The maximum amount (in GRR) to send to a single output address (default: 10000 GRR)')
parser.add_argument('-f', '--fee', type=Decimal, default=Decimal('1'),
  help='The amount of fees (in GRR) to use for the transaction')
...
print(f"\033[36mMax input amount:\033[0m {args.max_amt_input} GRR"
      f"\n\033[36mMax number of transactions to consolidate:\033[0m {args.max_num_tx}"
      f"\n\033[36mMax amount per output:\033[0m {args.max_amt_per_output} GRR"
      f"\n\033[36mFee:\033[0m {args.fee} GRR"
...
try:
  ...
  print("\033[91mCouldn't connect to GhostRiderCoin:", "\033[0m", e)
  sys.exit()
...
while True:
  ...
  for script in usescripts:
    print('  Script %s has %d txins and %s GRR value.' % (script, scripts[script][2], str(scripts[script][1])))

  out = {}
  na = amt - args.fee
  # One new output per max_amt_per_output GRR of value to avoid consolidating too much value in too few addresses.
  # But don't add an extra output if it would have less than args.max_amt_per_output GRR.
  while na > 0:
    ...
    if amount > 0:
      ...
      out[addr] += amount
    na -= amount
  print('Paying %s GRR (%s fee) to:' % (sum(out.values()), amt - sum(out.values())))
  for o in out.keys():
    print('  %s %s' % (o, out[o]))
  ...
  try:
    ...
    signed_txn = b.signrawtransactionwithwallet(txn)
